function [y, W, WW, Score] = sparseica_W_adasize_Alasso_mask_regu(lambda, Mask, x,regu)
...
WW = diag(1./std(xx'));
WW = natural_grad_Adasize_Mask_regu(xx, Mask,regu);
save WW_temp.mat WW;
...
regu_l1 = regu/2;
    grad_new = y_psi*x'/T + inv(W') -4*beta* (diag(diag(y*y'/T)) - eye(N)) * (y*x'/T) - dev*lambda/T  - 2*regu_l1 * W ;
    if iter==1
        grad_old = grad_new;
    end
